	...
	class numberField extends Field
	{
		...
		public function toForm($checkinform=true)
		{
			...
			$formfield=$formfield.'</label>
			    <input name="'.$this->_alias.'" type="number" placeholder="'.$this->_description.'"';
			if($this->_required)
			{
		...
		{
			$sqlcreate=$this->_alias." int(11) DEFAULT 0";
			if($this->_required)
		...
		public function toBackCell()
		{
			...
			{
				$texte='<input type="number" value="<?php echo $row->'.($this->getAlias()).';?>"/>';
			}
			return $texte;
		}
