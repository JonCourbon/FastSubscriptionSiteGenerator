	...
	class telField extends Field
	{
		...
		public function toForm($checkinform=true)
		{
			...
			$formfield=$formfield.'</label>
			    <input name="'.$this->_alias.'" type="tel" placeholder="'.$this->_description.'"';
			if($this->_required)
			{
		...
		{
			$sqlcreate=$this->_alias." varchar(255) DEFAULT 0";
		...
		public function toBackCell()
		{
			...
			{
				$texte='<input type="tel" value="<?php echo $row->'.($this->getAlias()).';?>"/>';
			}
			return $texte;
		}
